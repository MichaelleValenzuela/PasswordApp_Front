...
export const connectApiAdminResource = async (IS_AUTH: any, BODY: any, EXTRA: any, CLEAN: any, RESPONSE: any) => {
    ...
    await axios({
        ...
        if (BODY.method === "GET") {
            if (BODY.param === "admin/get-resources") {
                RESPONSE.DAPI_RESPONSE.value.isLoading = false;
                RESPONSE.DAPI_RESPONSE.value.response = res.data;
            }
            if (BODY.param.includes("admin/get-resource/")) {
                RESPONSE.DAPI_RESPONSE.value.isLoading = false;
                RESPONSE.DAPI_RESPONSE_ID.value.response = res.data;
            }
        }
        ...
        if (BODY.method === "PUT") {
            if (BODY.param.includes("admin/edit-type-encrypt-resource/")) {
                EXTRA.$toast.success(res.data.msg, {
                    position: 'top-right'
                });
            }
        ...
        if (BODY.method === "POST" || BODY.method === "PUT") {
            if (BODY.param === "admin/create-resource" || BODY.param.includes('admin/edit-resource/')) {
                CLEAN.form_data.value = { name_type_encrypt: "RFC-7519", name_resource: "" };
                CLEAN.mini_form_data_resource.value = { _id: "", name_input: "", type_input: "text" };
                CLEAN.isMiniFormDataEdit.value = false;
                CLEAN.arrMiniDataResource.value = [];
                EXTRA.onCloseModal();
                ...
                axios({
                    ...
                    .then(function (res1) {
                        RESPONSE.DAPI_RESPONSE.value.isLoading = false;
                        RESPONSE.DAPI_RESPONSE.value.response = res1.data;
                    });
